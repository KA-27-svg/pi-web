...
export function TrashList({ items, onRestore, onPurge, onEmpty }: TrashListProps) {
  ...
  if (items.length === 0) {
    return (
      <p className="px-2 py-3 text-[11px] leading-[1.7] text-muted">
        回收箱是空的。
        <br />
        删除的对话会在这里保留 30 天。
      </p>
        ...
        {items.map(item => {
          ...
          return (
            <li key={item.path} className="rounded-md px-2.5 py-2 hover:bg-surface">
              <div className="truncate text-[12px] text-foreground/85" title={title}>
                {title}
              </div>
      ...
      <div className="mt-2 border-t border-border px-2.5 pt-2.5 pb-1">
        {confirmingEmpty ? (
          ...
          <button
            ...
            className="text-[10.5px] text-muted transition-colors hover:text-foreground"
          >
            清空回收箱
          </button>
        )}
