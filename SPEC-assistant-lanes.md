...
第一版做的是「铺纸」：顾问栏从右边滑进来、分栏线像一笔墨画下来、一道光顺着扫过。
用户看过之后反馈「展开的动画很奇怪」，于是全部撤掉，只留按钮自己的点击动画。
教训记在这儿，免得以后再走一遍：

**一块空面板动起来就是很奇怪。** 顾问栏首屏常常还是空的（顾问进程要几秒才连上），
而且它跟主区域同色——不铺底色、不投影，那块空白滑进来在屏幕上等于什么都没发生；
真铺上底色和投影，又变成「一张纸在动」这种抢戏的东西。执行窗口和顾问栏是同一张纸的
两栏，不该有一栏先自己滑进来。

现在只有两段，都挂在按钮上，点击时各演一遍（`useIconPress`）：

| 按钮 | 做法 | 时长 |
|---|---|---|
| 助手模式开关 | `::after` 泛一圈涟漪（`scale` 0.6→2，透明度先升后落）+ 图标轻轻落定（`scale 0.8 / rotate -8deg` → 无） | 620 / 460ms |
| 设置齿轮 | 图标转一圈（`rotate` 0°→360°） | 520ms |

**为什么要有「动画窗口」这个状态**：动画带 `animation-fill-mode: both`，播完元素上
仍然留着属性值。窗口一过就摘类，元素回到干净的基础态。顺带一提，这些属性全用
`scale` / `rotate` 这两个独立变换属性、不写进 `transform`——万一以后按钮里塞进
`position: fixed` 的元素，残留的 `transform` 会把它的参照系从视口换成按钮。

**为什么由点击事件触发、而不是盯状态变化**（`useIconPress` 取代了原来的
`useAssistantOpening`）：图标动画只该跟着手指走。盯状态的话，页面加载时
（localStorage 里存着「开着」）会莫名其妙演一遍，别人改状态也会误触发。
`count` 每次点击自增、并当作图标元素的 `key`：连点两下时 `active` 本来就是 true、
React 不会重渲染，靠 `key` 变化重建图标元素，动画才会从头播。挂在图标上而不是按钮上，
是为了别把按钮本身重建掉（会丢焦点）。

`prefers-reduced-motion: reduce` 下全部 `animation: none`。剩下的反馈是按钮本来就有的
hover / 选中配色变化，够说明「点到了」，而旋转和缩放正是这个偏好想避开的东西。
