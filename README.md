...
日常开发用 `npm run dev`（热更新，两个进程）。想当成品跑：

```bash
npm start        # 先构建，再由桥接托管前端
```

之后**只有一个进程、一个端口**：打开 http://127.0.0.1:3001 就行，不再需要 Vite。
（`npm run serve` 跳过构建直接起，前提是已经构建过。）

从桥接端口访问前端不用改任何安全策略——Origin 白名单本来就把
`http://127.0.0.1:3001` 算在内。
