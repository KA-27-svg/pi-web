...
文件会上传到**当前工作目录**下的 `.pi-web-uploads/`，再把相对路径写进消息：

```
[附件] .pi-web-uploads/报告.docx

帮我总结这份文档
```

pi 的 RPC 没有通用附件接口（只支持图片），所以文件是**交给 agent 自己读**的——它有
`read` 和 `bash`，会按文件类型处理。这也是 Codex 的 `/mention` 同一思路：把路径递给
agent，而不是把文件塞给模型。

docx / pdf 这类二进制格式 **pi 自己不解析**，agent 通常会调用本机的 `pandoc` 转成文本，
所以装了 pandoc 体验会好很多（不装也能跑，agent 会试其它办法，只是可能不顺利）。

上传目录里会自动写一个 `.gitignore`（内容为 `*`），所以不会出现在项目 `git status` 里。
同名文件不覆盖，自动加 `-1` 后缀；单文件上限 25 MB；文件名会做净化（防路径穿越）。
...
- **图片不作原生附件**：截图也走上面那条文件路径，模型要自己读一次。pi 原生支持
  `prompt.images`，做成原生附件体验会更好（待办）。
