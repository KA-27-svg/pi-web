...
设置面板里可以打开只读模式：agent 手上只剩 `read` 和扩展提供的工具，
`bash` / `powershell` / `edit` / `write` 全部被排除（用的是 `--exclude-tools`）。

**能做**：问问题、审代码、查资料。**不能改文件、不能跑命令、生成不了任何文件。**

### 挡住什么，挡不住什么

| | 只读模式 | 真沙盒（容器 / 虚机） |
| --- | --- | --- |
| agent 改你的文件 | ✅ 做不到（工具不存在） | ✅ |
| agent 跑命令 | ✅ 做不到 | ✅ |
| agent **读**任意文件 | ❌ 仍然可以（`read` 没有路径边界） | ✅ |
| 扩展里的恶意代码 | ❌ 管不着（扩展以 pi 进程的权限运行） | ✅ |
| 桥接自己的宿主机能力 | ❌ 管不着 | 看桥接在哪一侧 |

**它不是沙盒。** pi 自己的文档里就有一节叫 [No Built-in Sandbox](https://github.com/earendil-works/pi-mono)：
真隔离必须来自操作系统或容器/虚机边界，而且要把 **pi 和桥接一起**关进去。
一个叫「安全模式」的开关会让人以为挡住了提示注入——那比不做更危险，所以这里叫「只读模式」。

### 两个实现细节

- 用 `--exclude-tools` 黑名单，不用 `--tools` 白名单：白名单会把扩展工具一起关掉
  （实测过，`exa_*` 全部消失）。
- 切换要重启 pi（工具集在启动时定死，RPC 没有改的接口），而**每次启动都是全新会话**，
  所以桥接会在重启后自动把当前会话切回来。

偏好存在 `~/.pi/agent/pi-web-prefs.json`——**不写进 pi 的全局 `settings.json`**，
免得从网页静默改掉你终端里的行为。
...
server/
  ...
  piWebPrefs.ts    读写 pi-web 自己的偏好（只读模式）
  openFile.ts      用系统默认程序打开附件
  reply.ts         统一的异步回包封装
